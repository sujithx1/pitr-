...
# Ensure log directory exists if writable, fallback to local log directory
...
# Execute backup wrapper with automated webhooks
if "$SCRIPT_DIR/backup_with_alert.sh" "$BACKUP_TYPE" 2>&1 | tee -a "$LOG_FILE"; then
